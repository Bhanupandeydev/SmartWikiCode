...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if (!a) {
                return message.channel.send(`${emotes.error} || **There Is No Modlog Channel Set To Disable!**`)
            } else {
                let channel = message.guild.channels.cache.get(a)
                client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("**ModLogs Channel Disabled!**")
                db.delete(`modlog_${message.guild.id}`)

                message.channel.send(`**Modlog Channel Has Been Successfully Disabled in \`${channel.name}\`**`).catch((e) => console.log(e) )
            }
            return
        } catch (err) {
