...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if (!a) {
                return message.channel.send(`${emotes.error} || **There Is No Muterole Set To Disable!**`)
            } else {
                let role = message.guild.roles.cache.get(a)
                db.delete(`muterole_${message.guild.id}`)

                message.channel.send(`${emotes.success} || **\`${role.name}\` Has Been Successfully Disabled**`)
            }
            return;
        } catch (err) {
