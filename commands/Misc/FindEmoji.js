...
module.exports = {
  name: "findemoji",
  description: "Shows the info about an emoji",
  category: "utility",
  aliases: ["findem", "findemote", "fem"],
...
let emojis = await fetch("https://emoji.gg/api/").then(res => res.json());
     ...
     if (!matches.length) return message.channel.send(`No results for **${args.join(" ")}**`)
     let page = 0;
     let poweredby = "emoji.gg";
     let embed = new discord.MessageEmbed()
     .setTitle(matches[page].title)
     .setURL("https://discordemoji.com/emoji/" + matches[page].slug)
     .setColor("YELLOW")
     .setImage(matches[page].image)
     .setFooter(`Emoji ${page + 1}/${matches.length} | Powered by ${poweredby}`);
     ...
     while (doing) {
     let reaction;
     try { reaction = await msg.awaitReactions(filter, { max: 1, time: 15000, errors: ["time"] })}
