// i am using a diff command handler change it by your own.
...
module.exports = {
    ...
    run: async (client, message, args) => {
        if(!args[0]) return message.channel.send("**Please mention someone to play with!**")
    let msg = message;
    ...
    if(member.bot) return message.channel.send("You can't duel a bot!")
    ...
    message.channel.send(`<@${author}>`, Embed).then(async message => {
      for (let i = 0; i < 9; i++) {
        ...
        try {
            ...
            if (
              ...
              if (a1 == "⭕" || a1 == "❌") {
                ...
                message.channel.send(
                  "That spot is already occupied.. and now you lost lol"
                );
                midDuel.delete(author);
                midDuel.delete(member.id);
                break;
          ...
          // Time limit ran out
          message.channel.send(
            `<@${turnName}> You took too long to respond, and now you lost. Nice!`
          );
          midDuel.delete(author);
          midDuel.delete(member.id);
          break;
