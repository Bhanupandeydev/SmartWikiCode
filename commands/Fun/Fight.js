module.exports = {
    name: "fight",
    description: "a fun fighting game!",
    group: "fun",
...
while (fighting) {
    ...
    try {
        ...
        message.channel.send(`**${opponent.username}** didn't answer in time, what a noob. **${opposite.username}** wins`);
        fighting = false;
        return;
    }
    msg = msg.first();
    ...
    if (content == "punch") {
        ...
        turn.hp += gain;
        message.channel.send(`PROTECC!! **${turn.username}** defended and now has \`${turn.hp} HP\` left!`);
        turn = opposite;
        continue;
    } else if (content == "end") {
        return message.channel.send(`**${turn.username}** has ended the game what a wimp`);
    }
}
}
}