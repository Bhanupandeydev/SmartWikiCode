...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!discrims.length) return message.reply('I cant find users with that discriminator');
    ...
    sourcebin.create([{
      name: `Discriminators Matching ${message.author.tag}` ,
      ...
      .setTitle(`Users Matching ${dis[0]} Discrim`)
      .setDescription(`${content}\n\nAdvanced List: **[Click Here](${src.url})**`)
      				.setThumbnail(message.author.displayAvatarURL({ dynamic: true }))
      .setFooter(`Found ${discrims.length} users matching ${dis[0]}`, client.user.displayAvatarURL())
