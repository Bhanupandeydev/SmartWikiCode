...
if (
  ...
  let mentionEmbed = new MessageEmbed()
    ...
    .addFields(
      ...
      {
        name: "Need support?",
        value: "Join our [support server](https://discord.gg/jp8et9xU56)",
      }
    )

  message.channel.send(mentionEmbed).catch((e) => {} )
    }
    ...
    if(checkingToS === null){
        let notAgreed = new Discord.MessageEmbed()
            ...
            .setColor(`#ee7373`)
            .setDescription(`By using SmartWiki bots features and adding this bot, you are agreeing to be bound by this bot's terms and conditions of use and agree that you are responsible for the agreement with any rules. If you disagree with any of these terms, you are prohibited from accessing this bot.`)
        ...
        return message.channel.send(notAgreed).then(async m => {
            ...
            const filter = (reaction, user) => {
                return ['SmartLike'].includes(reaction.emoji.name) && user.id === message.author.id
            }
   ...
   /*=-=-=-=-=-=-=-=-=-=-=-=CUSTOM CC [ENDS HERE]=-=-=-=-=-=-=-=-=-=-*/
    ...
    if(!command) {
 const best = [...client.commands.map(cmd => cmd.name), ...client.aliases.keys()].filter((c) => leven(cmd.toLowerCase(), c.toLowerCase()) < c.length * 0.3)

const dym = best.length == 0 ? '' : best.length == 1 ? `Did You Mean: \`${best[0]}\`` : `Did you mean: ${best.slice(0, 3).map(value => `\`${value}\``)}`;
return message.channel.send(new MessageEmbed({ color: `#ee7373`, description: `Unknown command. Use s!help to view the command list.\n${dym}`}));
    }
if (command) { 
/**==============[DISABLED COMMANDS]==============*/
const check = await db.findOne({ Guild: message.guild.id })
        if(check) {
            if(check.Cmds.includes(command.name)) return message.channel.send("This command has been disabled by an Admin")
        }
/*===============[ARGS]============*/
...
if (timestamps.has(message.author.id)) {
  const expirationTime = timestamps.get(message.author.id) + cooldownAmount;
    ...
    .setColor("RED")
    .setDescription(`You may not use the \`${command.name}\`command again for another ${timeLeft.toFixed(1)} seconds. \nSince you are waiting you can add **${client.user.username}** to your server by [clicking here!](https://dsc.gg/aiclever)`)
...
if (command.guildOnly && message.channel.type === 'dm') {
         ...
         return message.reply(`${emotes.error}Error: You Don't Have Enough Permissions To Use This Command ${command.permissions}`).catch((e) => {} )
     }
     } 
     if (command.OwnerOnly) {
        ...
        return message.reply(`Only ${client.user.username}(s) Developers access to run this command`).catch((e) => {} )
      }
          const guard = new Discord.MessageEmbed();
        guard.setTitle(`${emotes.error}Error: Beta Command!`)
        .setDescription(`Sorry, This Command Is Currently In Development So It Wont Work.`)
        if(command.guarded) {
...
blacklist.findOne({ id : message.author.id }, async(err, data) => {
            if(err) throw err;
            if(data) return message.channel.send(`You are blacklisted from Using \`${client.user.username}\`!`)
